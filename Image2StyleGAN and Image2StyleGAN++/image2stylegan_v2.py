...
class LayerEpilogue(nn.Module):
    ...
    def forward(self, x, dlatents_in_slice=None, noise_in_slice=None):
        x = self.top_epi[0](x, noise_in_slice)
        for i in range(1,len(self.top_epi)):
            x = self.top_epi[i](x)
...
class G_synthesis(nn.Module):
    ...
    def forward(self, dlatents_in, noise = None):
        batch_size = dlatents_in.size(0)

        for i, m in enumerate(self.blocks.values()):
            if i == 0:
                x = m(dlatents_in[:, 2 * i:2 * i + 2], noise[2*i:2*i+2])
            else:
                x = m(x, dlatents_in[:, 2 * i:2 * i + 2], noise[2*i:2*i+2])
